...
def extract_and_save_fft(train_eeg1, train_eeg2, train_emg, mode):
    ...
    for name, signals in zip(names, [train_eeg1, train_eeg2, train_emg]):
        ...
        for sig in tqdm(signals):
            sig_fft_feature = np.zeros((96, 16))
            for slice_idx in range(slice_size):
                start_pos = slice_idx*step_size
                slice = sig[start_pos:start_pos+window_size]
                slice_fft = np.abs(np.fft.rfft(slice)/window_size)
                slice_fft = slice_fft[:96]  # 0 - 48 Hz
                sig_fft_feature[:, slice_idx] = np.reshape(slice_fft, (96, 1))[:, 0]
...
def main():
    ...
    extract_and_save_fft(train_eeg1, train_eeg2, train_emg, 'train')
    extract_and_save_fft(test_eeg1, test_eeg2, test_emg, 'test')
